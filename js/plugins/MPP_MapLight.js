 ...
 *  ã€‡ MV / MZ
 *  
 *  ã€‡ SetCharLight evId r c a  / setCharLight
 ...
 *   
 *  ã€‡ SetPosLight x y r c a  / setPosLight
 *      x   : X coordinate
 *      y   : Y coordinate
 *      r   : radius(1 = 1 square)
 *      c   : color index(Erase with 0)
 ...
 * 
 * â–¼ Annotation of event execution content
 *  ã€‡ CharLight r c a
 *      r   : radius(1 = 1 square)
 *      c   : color index
 *      a   : amplitude(Specify from 0-100 / No blinking at 0 / Not set is 0)
...
/*:ja
 * @target MV MZ
 * @plugindesc ãƒžãƒƒãƒ—ã®æ˜Žã‚‹ã•ã‚’è¨­å®šã§ãã‚‹ã‚ˆã†ã«ã—ã¾ã™ã€‚
 ...
 * 
 * â–¼ ä»•æ§˜
 *  - ã‚­ãƒ£ãƒ©ã‚¯ã‚¿ãƒ¼ä¸€äººã«å¯¾ã—ã¦ä»˜åŠ ã§ãã‚‹ç¯ã‚Šã¯ã€ã‚­ãƒ£ãƒ©ç¯ã‚Šã¨å‰æ–¹ç¯ã‚Šãã‚Œãžã‚Œ
 *    ï¼‘ã¤ã¾ã§ã§ã™ã€‚
 *  - ã‚¤ãƒ™ãƒ³ãƒˆã‚„ãƒžãƒƒãƒ—ä¸Šã«ä½œæˆã—ãŸç¯ã‚Šã¯ã€ãƒžãƒƒãƒ—åˆ‡ã‚Šæ›¿ãˆã§ãƒªã‚»ãƒƒãƒˆã•ã‚Œã¾ã™ã€‚
 *  - ãƒ—ãƒ¬ã‚¤ãƒ¤ãƒ¼ã®ç¯ã‚Šã¯ãƒžãƒƒãƒ—ç§»å‹•ã—ã¦ã‚‚ç¶­æŒã•ã‚Œã¾ã™ã€‚
 * 
 * â–¼ ãƒ—ãƒ©ã‚°ã‚¤ãƒ³ã‚³ãƒžãƒ³ãƒ‰
 *  - MVã§ã¯æ•°å€¤ã‚’å…¥åŠ›ã™ã‚‹é …ç›®ã§ v[N] ã¨è¨˜è¿°ã™ã‚‹ã“ã¨ã§å¤‰æ•°Nç•ªã‚’å‚ç…§ã—ã¾ã™ã€‚
 *  - MZã§ã¯æ•°å€¤ã‚’å…¥åŠ›ã™ã‚‹é …ç›®ã§ã€ãƒ†ã‚­ã‚¹ãƒˆã‚’é¸æŠžã—ã¦ v[N] ã¨è¨˜è¿°ã™ã‚‹ã“ã¨ã§
 *    å¤‰æ•°Nç•ªã‚’å‚ç…§ã—ã¾ã™ã€‚
 *  - è‰²ç•ªå·(c)ã¯ãƒ—ãƒ©ã‚°ã‚¤ãƒ³ãƒ‘ãƒ©ãƒ¡ãƒ¼ã‚¿[Light Colors]ã§è¨­å®šã—ãŸã‚‚ã®ã‚’æŒ‡å®šã—ã¦ãã ã•ã„ã€‚
 *  
 *  ã€‡ MV / MZ
 *  
 *  ã€‡ SetCharLight evId r c a  / ã‚­ãƒ£ãƒ©ç¯ã‚Šè¨­å®š
 *      evId : ã‚¤ãƒ™ãƒ³ãƒˆID(-1:ãƒ—ãƒ¬ã‚¤ãƒ¤ãƒ¼, 0:ã“ã®ã‚¤ãƒ™ãƒ³ãƒˆ)
 *      r    : åŠå¾„(1ã§1ãƒžã‚¹åˆ†)
 *      c    : è‰²ç•ªå·(0ã§æ¶ˆåŽ»)
 *      a    : æ˜Žæ»…ã®æŒ¯ã‚Œå¹…(0ï½ž100ã§æŒ‡å®š / 0ã§æ˜Žæ»…ãªã— / æœªè¨­å®šã¯0)
 *   - ã‚­ãƒ£ãƒ©ã‚¯ã‚¿ãƒ¼ã‚’ä¸­å¿ƒã«ç¯ã‚Šã‚’è¡¨ç¤ºã—ã¾ã™ã€‚
 *   
 *  ã€‡ SetPosLight x y r c a  / ãƒžãƒƒãƒ—ç¯ã‚Šè¨­å®š
 *      x   : Xåº§æ¨™
 *      y   : Yåº§æ¨™
 *      r   : åŠå¾„(1ã§1ãƒžã‚¹åˆ†)
 *      c   : è‰²ç•ªå·(0ã§æ¶ˆåŽ»)
 *      a   : æ˜Žæ»…ã®æŒ¯ã‚Œå¹…(0ï½ž100ã§æŒ‡å®š / 0ã§æ˜Žæ»…ãªã— / æœªè¨­å®šã¯0)
 *   - æŒ‡å®šã—ãŸåº§æ¨™(x,y)ã«ç¯ã‚Šè¡¨ç¤ºã—ã¾ã™ã€‚
 *   
 *  ã€‡ SetFrontLight evId r c a  / å‰æ–¹ç¯ã‚Šè¨­å®š
 *      evId : ã‚¤ãƒ™ãƒ³ãƒˆID(-1:ãƒ—ãƒ¬ã‚¤ãƒ¤ãƒ¼, 0:ã“ã®ã‚¤ãƒ™ãƒ³ãƒˆ)
 *      c    : è‰²ç•ªå·(0ã§æ¶ˆåŽ»)
 *      a    : æ˜Žæ»…ã®æŒ¯ã‚Œå¹…(0ï½ž100ã§æŒ‡å®š / 0ã§æ˜Žæ»…ãªã— / æœªè¨­å®šã¯0)
 *   - ã‚­ãƒ£ãƒ©ã‚¯ã‚¿ãƒ¼ã®å‰æ–¹ã«ç¯ã‚Šã‚’è¡¨ç¤ºã—ã¾ã™ã€‚
 *   
 *  ã€‡ EraseCharLight lId  / ã‚­ãƒ£ãƒ©ç¯ã‚Šæ¶ˆåŽ»
 *      evId : ã‚¤ãƒ™ãƒ³ãƒˆID(-1:ãƒ—ãƒ¬ã‚¤ãƒ¤ãƒ¼, 0:ã“ã®ã‚¤ãƒ™ãƒ³ãƒˆ)
 *   - ã‚­ãƒ£ãƒ©ã‚¯ã‚¿ãƒ¼ã‚’ä¸­å¿ƒã¨ã—ãŸç¯ã‚Šã‚’æ¶ˆåŽ»ã—ã¾ã™ã€‚
 * 
 *  ã€‡ ErasePosLight x y  / ãƒžãƒƒãƒ—ç¯ã‚Šæ¶ˆåŽ»
 *      x   : Xåº§æ¨™
 *      y   : Yåº§æ¨™
 *   - æŒ‡å®šã—ãŸåº§æ¨™(x,y)ã®ç¯ã‚Šã‚’æ¶ˆåŽ»ã—ã¾ã™ã€‚
 * 
 *  ã€‡ EraseFrontLight lId  / å‰æ–¹ç¯ã‚Šæ¶ˆåŽ»
 *      evId : ã‚¤ãƒ™ãƒ³ãƒˆID(-1:ãƒ—ãƒ¬ã‚¤ãƒ¤ãƒ¼, 0:ã“ã®ã‚¤ãƒ™ãƒ³ãƒˆ)
 *   - ã‚­ãƒ£ãƒ©ã‚¯ã‚¿ãƒ¼ã®å‰æ–¹ã‚’ç…§ã‚‰ã™ç¯ã‚Šã‚’æ¶ˆåŽ»ã—ã¾ã™ã€‚
 * 
 *  ã€‡ SetMapDarkness d  / ãƒžãƒƒãƒ—æš—ã•è¨­å®š
 *      d : ãƒžãƒƒãƒ—ã®æš—ã•(0ï½ž100ã§æŒ‡å®š)
 *   - ãƒžãƒƒãƒ—ã®æš—ã•ã‚’å¤‰æ›´ã—ã¾ã™ã€‚
 * 
 * â–¼ ãƒžãƒƒãƒ—ã®ãƒ¡ãƒ¢
 *  ã€‡ <Darkness:n>
 *      n : ãƒžãƒƒãƒ—ã®æš—ã•(0ï½ž100ã§æŒ‡å®š)
 *   - ãƒžãƒƒãƒ—ã®æš—ã•ã‚’æŒ‡å®šã—ã¾ã™ã€‚
 *   - v[n] ã¨è¨˜è¿°ã™ã‚‹ã“ã¨ã§nç•ªã®å¤‰æ•°ã®å€¤ã‚’å‚ç…§ã—ã¾ã™ã€‚
 *   - ãŸã ã—ã€å‚ç…§ã•ã‚Œã‚‹ã®ã¯ãƒžãƒƒãƒ—ã®åˆ‡ã‚Šæ›¿ãˆã‚’è¡Œã£ãŸã¨ãã®ã¿ã§ã™ã€‚
 * 
 *  ã€‡ <PosLight X,Y:r,c,a>
 *      X   : Xåº§æ¨™
 *      Y   : Yåº§æ¨™
 ...
 *      c   : è‰²ç•ªå·
 *      a   : æ˜Žæ»…ã®æŒ¯ã‚Œå¹…(0ï½ž100ã§æŒ‡å®š / 0ã§æ˜Žæ»…ãªã— / æœªè¨­å®šã¯0)
 *   - æŒ‡å®šã—ãŸåº§æ¨™(x,y)ã«ç¯ã‚Šã‚’é…ç½®ã—ã¾ã™ã€‚
 * 
 * â–¼ ã‚¤ãƒ™ãƒ³ãƒˆã®å®Ÿè¡Œå†…å®¹ã®æ³¨é‡ˆ
 *  ã€‡ ç¯ã‚Š r c a
 *      r   : åŠå¾„(1ã§1ãƒžã‚¹åˆ†)
 *      c   : è‰²ç•ªå·
 *      a   : æ˜Žæ»…ã®æŒ¯ã‚Œå¹…(0ï½ž100ã§æŒ‡å®š / 0ã§æ˜Žæ»…ãªã— / æœªè¨­å®šã¯0)
 *   - ã“ã®ã‚¤ãƒ™ãƒ³ãƒˆã‚’ä¸­å¿ƒã«ç¯ã‚Šã‚’è¡¨ç¤ºã—ã¾ã™ã€‚
 *   - ã“ã®ã‚³ãƒžãƒ³ãƒ‰ã¯å®Ÿè¡Œå†…å®¹ã®æœ€åˆã®æ³¨é‡ˆã®ã¿é©ç”¨ã•ã‚Œã¾ã™ã€‚
 *  
 *  ã€‡ å‰æ–¹ç¯ã‚Š c a
 *      c   : è‰²ç•ªå·
 *      a   : æ˜Žæ»…ã®æŒ¯ã‚Œå¹…(0ï½ž100ã§æŒ‡å®š / 0ã§æ˜Žæ»…ãªã— / æœªè¨­å®šã¯0)
 *   - ã“ã®ã‚¤ãƒ™ãƒ³ãƒˆã®å‰æ–¹ã‚’ç…§ã‚‰ã™ç¯ã‚Šã‚’è¡¨ç¤ºã—ã¾ã™ã€‚
 *   - ã“ã®ã‚³ãƒžãƒ³ãƒ‰ã¯å®Ÿè¡Œå†…å®¹ã®æœ€åˆã®æ³¨é‡ˆã®ã¿é©ç”¨ã•ã‚Œã¾ã™ã€‚
 *  
 * ================================
 ...
 * 
 *  @command setCharLight
 *      @text ã‚­ãƒ£ãƒ©ç¯ã‚Šè¨­å®š
 *      @desc 
 *      @arg character
 *          @text å¯¾è±¡ã‚­ãƒ£ãƒ©ã‚¯ã‚¿ãƒ¼
 *          @desc -1:ãƒ—ãƒ¬ã‚¤ãƒ¤ãƒ¼, 0:ã“ã®ã‚¤ãƒ™ãƒ³ãƒˆ, 1..:ã‚¤ãƒ™ãƒ³ãƒˆID
 *          @type number
 *              @min -1
 *              @max 999
 *          @default 0
 ...
 *      @arg colorIndex
 *          @text è‰²ç•ªå·
 *          @desc ãƒ—ãƒ©ã‚°ã‚¤ãƒ³ãƒ‘ãƒ©ãƒ¡ãƒ¼ã‚¿[Light Colors]ã§æŒ‡å®š / 0:æ¶ˆåŽ»
 *          @type number
 *              @min 0
 *              @max 99
 *          @default 1
 ...
 * 
 *  @param Light Colors
 *      @text ç¯ã‚Šã®è‰²ã®é…åˆ—
 *      @desc 
 *      @type string[]
 *      @default ["255,255,255","192,128,64","32,32,32","192,192,192"]
 * 
 *  @param Front Light
 *      @text å‰æ–¹ç¯ã‚Šãƒ‘ãƒ©ãƒ¡ãƒ¼ã‚¿
 *      @desc 
 *      @type struct<FrontLight>
 *      @default {"Radius":"6","Angle":"90","Oy":"4","Turn Duration":"24"}
 * 
 *  @param Darkness Size
 *      @text æš—ã•ã®è§£åƒåº¦
 *      @desc 1ã‚¿ã‚¤ãƒ«å½“ãŸã‚Šã®è§£åƒåº¦
 *      @type number
 *          @min 1
 *          @max 48
 *      @default 3
 *
 *  @param Additive Alpha
 *      @text è‰²åŠ ç®—ã‚¢ãƒ«ãƒ•ã‚¡å€¤
 *      @desc ç¯ã‚Šã®è‰²ã‚’åŠ ç®—ã§è¡¨ç¤ºã™ã‚‹ãƒ¬ã‚¤ãƒ¤ãƒ¼ã®ä¸é€æ˜ŽçŽ‡ã€‚
 * 0:ãªã—, 100:æœ€å¤§
 *      @type number
 *          @min 0
 *          @max 100
...
(() => {
    ...
    DarknessLayer.prototype._paintFrontLight = function(x, y, rgb, angle, alpha) {
        ...
        // MVã§ size = 4, angle = 0 ã®å ´åˆã€ä»–ã®å€¤ã®å°æ•°ç‚¹ä»¥ä¸‹ãŒç„¡è¦–ã•ã‚Œã‚‹ãŸã‚ã€ãã®å¯¾å‡¦
    ...
    Game_Event.prototype.setupCharLight = function() {
        const comment = this.getCommentCommand('CharLight', 'ç¯ã‚Š');
        if (comment) {
    ...
    Game_Event.prototype.setupFrontLight = function() {
        const comment = this.getCommentCommand('FrontLight', 'å‰æ–¹ç¯ã‚Š');
        if (comment) {
            const [_, ...args] = comment.split(' ');
    ...
    Object.assign(_mzCommands, {
        'ã‚­ãƒ£ãƒ©ç¯ã‚Šè¨­å®š': _mzCommands.SetCharLight,
        'ãƒžãƒƒãƒ—ç¯ã‚Šè¨­å®š': _mzCommands.SetPosLight,
        'å‰æ–¹ç¯ã‚Šè¨­å®š': _mzCommands.SetFrontLight,
        'ã‚­ãƒ£ãƒ©ç¯ã‚Šæ¶ˆåŽ»': _mzCommands.EraseCharLight,
        'ãƒžãƒƒãƒ—ç¯ã‚Šæ¶ˆåŽ»': _mzCommands.ErasePosLight,
        'å‰æ–¹ç¯ã‚Šæ¶ˆåŽ»': _mzCommands.EraseFrontLight,
        'ãƒžãƒƒãƒ—æš—ã•è¨­å®š': _mzCommands.SetMapDarkness
    });
